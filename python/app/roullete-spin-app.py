...
def spin():
    ...
    winning_field = requests.get(
        args.random_field_api, headers=tracking_headers
    ).content
    winning_field = json.loads(winning_field)

    # notify payout
    requests.post(args.payout_api, headers=tracking_headers, json=winning_field)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--random-field-api", action="store", required=True, dest="random_field_api"
    )
    parser.add_argument(
        "--payout-api", action="store", required=True, dest="payout_api"
    )
